  ...
  for (const auto& typeMeta : typeMetas) {
    ...
    if (typeMeta.HaveAnyPublicField()) {
      ...
      for (const auto& field : typeMeta.fields) {
        ...
        ss << attr.GenerateName(attr.ns.empty()
        ...
        switch (field.mode) {
          ...
          case Field::Mode::Function: {
            if (field.name == typeMeta.name) {
              // constructor
              ss << "WrapConstructor<" << tname << "("
                 << field.GenerateParamTypeList() << ")>()";
            } else if (field.name == ("~" + typeMeta.name)) {
              // destructor
              ss << "WrapDestructor<" << tname << ">()";
            } else if (typeMeta.IsOverloaded(field.name))
              ss << "static_cast<" << field.GenerateFunctionType(tname) << ">(&"
                 << tname << "::" << field.name << ")";
            else
              ss << "&" << tname << "::" << field.name;
            break;
        ...
        const bool hasDefaultFunctionsAttr =
            config.generateDefaultFunctions &&
            field.mode == Field::Mode::Function &&
            field.GetDefaultParameterNum() > 0;
        if (!field.attrs.empty() || hasInitializerAttr ||
            hasDefaultFunctionsAttr) {
          ss << ", AttrList {" << endl;
          // [initializer]
