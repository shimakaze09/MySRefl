...
struct MetaGenerator::Impl : CPP14ParserBaseVisitor {
  ...
  std::string GetTextPro(antlr4::ParserRuleContext* ctx) const {
    return {code.begin() + ctx->getStart()->getStartIndex(),
  ...
  antlrcpp::Any visitAttributeSpecifierSeq(
      CPP14Parser::AttributeSpecifierSeqContext* ctx) override final {
    std::vector<Attr> attrs;
    for (auto* ctxAttributeSpecifier : ctx->attributeSpecifier()) {
      std::vector<Attr> partialAttrs = visit(ctxAttributeSpecifier);
      for (auto& attr : partialAttrs) attrs.push_back(std::move(attr));
    }
    return attrs;
  }

  // attributeSpecifier:
  //     LeftBracket LeftBracket(Using attributeNamespace Colon) ? attributeList
  //     ? RightBracket RightBracket
  //	   | alignmentspecifier
  antlrcpp::Any visitAttributeSpecifier(
      CPP14Parser::AttributeSpecifierContext* ctx) override final {
    std::vector<Attr> attrs;
    ...
    if (ctx->attributeList()) {
      ...
      if (ctx->attributeNamespace()) {
        ...
        for (auto& attr : attrs) attr.ns = ns;
      }
    }
  ...
  antlrcpp::Any visitAttribute(
      CPP14Parser::AttributeContext* ctx) override final {
    Attr attr;
    if (ctx->attributeNamespace())
      attr.ns = ctx->attributeNamespace()->getText();
    attr.name = ctx->Identifier()->getText();
    if (ctx->attributeArgumentClause() &&
        ctx->attributeArgumentClause()->balancedTokenSeq())
      attr.value =
          ctx->attributeArgumentClause()->balancedTokenSeq()->getText();
    return attr;
  }

  antlrcpp::Any visitTypeParameter(
      CPP14Parser::TypeParameterContext* ctx) override final {
    Parameter parameter;
    if (ctx->Typename_())
    ...
    if (ctx->Ellipsis()) parameter.isPacked = true;
    if (ctx->Identifier()) parameter.name = ctx->Identifier()->getText();
    return parameter;
  }

  antlrcpp::Any visitPointerAbstractDeclarator(
      CPP14Parser::PointerAbstractDeclaratorContext* ctx) override final {
  ...
  antlrcpp::Any visitAbstractPackDeclarator(
      CPP14Parser::AbstractPackDeclaratorContext* ctx) override final {
    Declarator declarator;
   ...
   *     );
   */
  antlrcpp::Any visitParameterDeclaration(
      CPP14Parser::ParameterDeclarationContext* ctx) override final {
    Parameter parameter;
    std::size_t start = ctx->proDeclSpecifierSeq()->preDeclSpecifierSeq()
                            ? ctx->proDeclSpecifierSeq()
                                  ->preDeclSpecifierSeq()
                                  ->getStart()
                                  ->getStartIndex()
                            : ctx->proDeclSpecifierSeq()
   ...
   *    )? enumbase?;
   */
  antlrcpp::Any visitEnumSpecifier(
      CPP14Parser::EnumSpecifierContext* ctx) override final {
    if (inTemplateType) return {};

    if (!ctx->enumHead()->Identifier()) return {};
    ...
    if (ctx->enumHead()->attributeSpecifierSeq())
      typeMeta.attrs = visit(ctx->enumHead()->attributeSpecifierSeq())
                           .as<std::vector<Attr>>();
  ...
  antlrcpp::Any visitEnumeratorList(
      CPP14Parser::EnumeratorListContext* ctx) override final {
  ...
  // enumeratorDefinition: enumerator attributeSpecifierSeq? (Assign
  // constantExpression)?
  antlrcpp::Any visitEnumeratorDefinition(
      CPP14Parser::EnumeratorDefinitionContext* ctx) override final {
    Field field;
  ...
  antlrcpp::Any visitTemplateDeclaration(
      CPP14Parser::TemplateDeclarationContext* ctx) override final {
    templateParameters =
        visit(ctx->templateparameterList()).as<std::vector<Parameter>>();
    ...
    if (rst.is<std::vector<Field>>()) {
      for (auto& field : rst.as<std::vector<Field>>()) field.isTemplate = true;
  ...
  antlrcpp::Any visitTemplateparameterList(
      CPP14Parser::TemplateparameterListContext* ctx) override final {
  ...
  antlrcpp::Any visitClassSpecifier(
      CPP14Parser::ClassSpecifierContext* ctx) override final {
    if (inTemplateType) return {};
    ...
    if (ctx->classHead()->attributeSpecifierSeq())
      typeMeta.attrs = visit(ctx->classHead()->attributeSpecifierSeq())
                           .as<std::vector<Attr>>();

    typeMeta.templateParameters = std::move(templateParameters);
    if (!typeMeta.templateParameters.empty()) inTemplateType = true;
    ...
    if (ctx->classHead()->baseClause())
      typeMeta.bases =
          visit(ctx->classHead()->baseClause()).as<std::vector<Base>>();
  ...
  antlrcpp::Any visitBaseClause(
      CPP14Parser::BaseClauseContext* ctx) override final {
    std::vector<Base> bases;
    for (auto* ctxBaseSpecifierX : ctx->baseSpecifierList()->baseSpecifierX()) {
      Base base;
      if (ctxBaseSpecifierX->Ellipsis()) base.isPacked = true;
      base.name =
          GetTextPro(ctxBaseSpecifierX->baseSpecifier()->baseTypeSpecifier());
      if (ctxBaseSpecifierX->baseSpecifier()->Virtual()) base.isVirtual = true;
      if (auto ctxAccessSpecifier =
              ctxBaseSpecifierX->baseSpecifier()->accessSpecifier()) {
  ...
  antlrcpp::Any visitAccessSpecifier(
      CPP14Parser::AccessSpecifierContext* ctx) override final {
    if (!accessSpecifiers.empty()) {
      if (ctx->Public())
        accessSpecifiers.back() = AccessSpecifier::PUBLIC;
  ...
  antlrcpp::Any visitNamespaceDefinition(
      CPP14Parser::NamespaceDefinitionContext* ctx) override final {
    ...
    if (ctx->Identifier())
      ...
      std::vector<std::string> curNamespaces =
          visitOriginalNamespaceName(ctx->originalNamespaceName());
      for (auto& ns : curNamespaces) namespaces.push_back(std::move(ns));
    }

    visit(ctx->declarationseq());

    while (namespaces.size() > origNamespacesSize) namespaces.pop_back();

    return {};
  }

  antlrcpp::Any visitNestedNameSpecifier(
      CPP14Parser::NestedNameSpecifierContext* ctx) override final {
    if (ctx->nestedNameSpecifier()) {
      std::vector<std::string> curNamespaces =
          visitNestedNameSpecifier(ctx->nestedNameSpecifier());
  ...
  antlrcpp::Any visitQualifiedNamespaceSpecifier(
      CPP14Parser::QualifiedNamespaceSpecifierContext* ctx) override final {
    if (ctx->nestedNameSpecifier()) {
      std::vector<std::string> curNamespaces =
          visitNestedNameSpecifier(ctx->nestedNameSpecifier());
  ...
  antlrcpp::Any visitProDeclSpecifierSeq(
      CPP14Parser::ProDeclSpecifierSeqContext* ctx) override final {
    ...
    if (ctx->attributeSpecifierSeq()) {
      ...
      for (auto& attr : attrs) seq.attrs.push_back(std::move(attr));
    }
    return seq;
  }

  antlrcpp::Any visitMemberDeclaration(
      CPP14Parser::MemberDeclarationContext* ctx) override final {
    if (ctx->emptyDeclaration() || ctx->aliasDeclaration() ||
        ctx->staticAssertDeclaration() || ctx->usingDeclaration())
      return {};

    if (ctx->declSpecifierSeq()) return visit(ctx->declSpecifierSeq());
    if (ctx->templateDeclaration()) return visit(ctx->templateDeclaration());
    ...
    if (ctx->attributeSpecifierSeq()) {
      std::vector<Attr> attrs = visit(ctx->attributeSpecifierSeq());
      for (auto& attr : attrs) commonAttrs.push_back(std::move(attr));
    }
    ...
    if (ctx->memberDeclaratorList()) {
      ...
      for (auto& declarator : declarators) {
        ...
        if (field.IsStaticConstexprVariable()) field.mode = Field::Mode::Value;
        for (const auto& attr : commonAttrs) field.attrs.push_back(attr);
        fields.push_back(std::move(field));
      }
    }
  ...
  antlrcpp::Any visitMemberDeclaratorList(
      CPP14Parser::MemberDeclaratorListContext* ctx) override final {
   ...
   */
  antlrcpp::Any visitMemberDeclarator(
      CPP14Parser::MemberDeclaratorContext* ctx) override final {
    Declarator declarator;
    if (ctx->declarator())
      ...
      if (ctx->attributeSpecifierSeq())
        declarator.attrs =
            visit(ctx->attributeSpecifierSeq()).as<std::vector<Attr>>();
    } else
      assert(false);

    if (ctx->braceOrEqualInitializer())
      declarator.initializer =
          visit(ctx->braceOrEqualInitializer()).as<std::string>();

    return declarator;
  }

  antlrcpp::Any visitPointerDeclarator(
      CPP14Parser::PointerDeclaratorContext* ctx) override final {
  ...
  antlrcpp::Any visitNoPointerDeclarator(
      CPP14Parser::NoPointerDeclaratorContext* ctx) override final {
    Declarator declarator =
    ...
    if (ctx->attributeSpecifierSeq()) {
      std::vector<Attr> attrs = visit(ctx->attributeSpecifierSeq());
      for (auto& attr : attrs) declarator.attrs.push_back(std::move(attr));
    }
    ...
    if (ctx->parametersAndQualifiers()) {
      ...
      if (ctx->parametersAndQualifiers()->attributeSpecifierSeq()) {
        std::vector<Attr> attrs =
            visit(ctx->parametersAndQualifiers()->attributeSpecifierSeq());
        for (auto& attr : attrs) declarator.attrs.push_back(std::move(attr));
      }
      if (auto* ctxClause =
              ctx->parametersAndQualifiers()->parameterDeclarationClause()) {
        for (auto* ctxParameterDeclaration :
             ctxClause->parameterDeclarationList()->parameterDeclaration()) {
      ...
      if (ctx->parametersAndQualifiers()->exceptionSpecification())
        declarator.qualifiers.push_back(ctx->parametersAndQualifiers()
  ...
  antlrcpp::Any visitBraceOrEqualInitializer(
      CPP14Parser::BraceOrEqualInitializerContext* ctx) override final {
    std::string rst;
    if (ctx->initializerClause())
  ...
  antlrcpp::Any visitSimpleDeclaration(
      CPP14Parser::SimpleDeclarationContext* ctx) override final {
    if (!ctx->initDeclaratorList()) return visitChildren(ctx);
  ...
  antlrcpp::Any visitInitDeclaratorList(
      CPP14Parser::InitDeclaratorListContext* ctx) override final {
    std::vector<Declarator> declarators;
  ...
  antlrcpp::Any visitInitDeclarator(
      CPP14Parser::InitDeclaratorContext* ctx) override final {
    Declarator declarator = visit(ctx->declarator());
    return declarator;
  }

  // functionDefinition: attributeSpecifierSeq ? proDeclSpecifierSeq ?
  // declarator virtualSpecifierSeq ? functionBody;
  antlrcpp::Any visitFunctionDefinition(
      CPP14Parser::FunctionDefinitionContext* ctx) override final {
    Declarator declarator = visit(ctx->declarator());
    Field field = std::move(declarator).GenerateField();
    if (!accessSpecifiers.empty())
      field.accessSpecifier = accessSpecifiers.back();
    ...
    if (ctx->proDeclSpecifierSeq()) {
      ...
      for (auto& attr : seq.attrs) field.attrs.push_back(std::move(attr));
    }
    if (ctx->functionBody()->Delete()) field.initializer = "delete";
    return field;
  }
};
