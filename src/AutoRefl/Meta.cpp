...
std::string Attr::GenerateValue(const std::string& type,
                                bool toFunction) const {
  if (toFunction)
...
std::string Field::GenerateSimpleFieldType() const {
  ...
  for (size_t i = 0; i < declSpecifiers.size(); i++) {
    ...
    if (
        ...
        declSpecifier != "register" && declSpecifier != "static" &&
        declSpecifier != "thread_local" && declSpecifier != "extern" &&
        declSpecifier != "mutable"

        // function specifier
        && declSpecifier != "inline" && declSpecifier != "virtual" &&
        declSpecifier != "explicit"

        // others
        && declSpecifier != "friend" && declSpecifier != "typedef" &&
...
std::string Field::GenerateInitFunction() const {
  return "[]{ return " + GenerateSimpleFieldType() +
