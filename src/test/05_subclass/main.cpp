...
struct Type<C> {
  ...
  static constexpr FieldList fields = {Field{"c", &C::c, AttrList{}}};
...
struct Type<D> {
  ...
  static constexpr FieldList fields = {Field{"d", &D::d, AttrList{}}};
...
void dump() {
  cout << Type<T>::name << "(";
  Type<T>::subclasses.ForEach(
      [](auto t) { dump<typename decltype(t)::type>(); });
  cout << ")";
}
