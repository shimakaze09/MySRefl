...
constexpr void ForEach(const List& list, const Func& func,
...
struct TypeList;  // forward declaration
...
constexpr void ForEachVarOf(T&& obj, const Func& func) {
  auto rec_func = [&](auto&& rec_obj) {
    Type<std::decay_t<decltype(rec_obj)>>::fields.ForEach(
        [&inner_obj = rec_obj, &func](auto field) {
