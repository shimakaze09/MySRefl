...
#define MYSREFL_FIELD_TYPE_OF(ptr)                                  \
  My::MySRefl::Field<                                               \
      typename My::MySRefl::FieldTraits<decltype(ptr)>::value_type, \
      typename My::MySRefl::FieldTraits<decltype(ptr)>::object_type, ptr>

#define MYSREFL_FIELD_OF(ptr) \
  MYSREFL_FIELD_TYPE_OF(ptr) {}
...
struct Field {
  ...
  static constexpr void ForEachMeta(const Func& func) {
    for (const auto& meta : metas) func(meta);
  }
};
...
static constexpr void ForEachFieldOf(T&& obj, const Func& func) {
  ForEachFieldOf(std::forward<T>(obj), func,
                 std::make_index_sequence<Type<std::decay_t<T>>::numFields>{});
}
