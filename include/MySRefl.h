...
static constexpr void ForEachField(const Fields& fields, const Func& func,
                                   std::index_sequence<Ns...>) {
  (func(std::get<Ns>(fields)), ...);
}

template <typename AttrList, typename Func, size_t... Ns>
static constexpr void ForEachAttr(const AttrList& attrs, const Func& func,
                                  std::index_sequence<Ns...>) {
...
static constexpr size_t FindIfField(const FieldList& fields, const Func& func,
                                    std::index_sequence<Ns...>) {
  if constexpr (sizeof...(Ns) == 0)
    ...
    return func(std::get<head>(fields)) ? head
                                        : FindIfField(fields, func, tail{});
  }
}

template <typename AttrList, typename Func, size_t... Ns>
static constexpr size_t FindIfAttr(const AttrList& attrs, const Func& func,
                                   std::index_sequence<Ns...>) {
  if constexpr (sizeof...(Ns) == 0)
    return false;
  else {
    constexpr size_t head =
        IndexSequenceTraits<std::index_sequence<Ns...>>::head;
    using tail = typename IndexSequenceTraits<std::index_sequence<Ns...>>::tail;
    return func(std::get<head>(attrs)) ? head : FindIfAttr(attrs, func, tail{});
  }
}
}  // namespace My::MySRefl::detail
...
struct Attr {
  constexpr Attr(std::string_view key, T value) noexcept
      : key{key}, value{value} {}
...
struct AttrList : std::tuple<Attrs...> {
  ...
  constexpr size_t FindIf(const Func& func) const {
    return detail::FindIfAttr(*this, func,
...
struct Field : FieldTraits<T> {
  constexpr Field(std::string_view name, T ptr, AList attrs) noexcept
...
struct FieldList : std::tuple<Fields...> {
  ...
  template <typename Func>
  constexpr size_t FindIf(const Func& func) const {
    return detail::FindIfField(*this, func,
